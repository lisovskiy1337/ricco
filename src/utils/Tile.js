...
export const TileWrapper = ({children, numOfPages}) => {
    ...
    return (
        <TileContext.Provider value={{numOfPages, currentPage}}>
            <div ref={refContainer} className="relative z-10 bg-black text-white" style={{height : numOfPages * 100 + 'vh'}}>{children}</div>
        </TileContext.Provider>
    )
}

export const TileBackground = ({children}) => (
        <div className="absolute h-full w-full z-10">{children}</div>
)
...
export const Tile = ({ page, renderContent }) => {
    ...
    return (
        <div ref={refContainer} className="absolute top-0 w-full z-10" style={{
